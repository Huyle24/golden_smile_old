                    ...
                    {listInvoice && listInvoice.length > 0 ? listInvoice.map((item, index) => (
                            ...
                            <Card className={'mt-3'}>
                                <Card.Body className={'body-card-invoice'}>
                                    <Row>
                                        <Col md={3}>
                                            <img className={'image-invoice'} src={item.tour_image?item.tour_image:"https://vigomanager.com/app-assets/mobile/img-huy/golden%20smile%20logo.png"} alt=""/>
                                        </Col>
                                        <Col md={9}>
                                            <div className={'d-flex justify-content-between'}>
                                                <div className={'tour-name'}>{item.tour_name?item.tour_name:""}</div>
                                                <div> <span className={'status'} style={{color:item.status_text_color, background:item.status_bg_color}}> {item.status_name}</span></div>
                                            </div>
