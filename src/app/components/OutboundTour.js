...
function OutboundTour(props) {
    useEffect(() => {
        props.fetchOutboundTourList('', '', '', '', '', '', '', '','','','','','',1,'' )
    }, [])

    let list_tour = props.outboundListInfo.data && props.outboundListInfo.isLoading === false ? props.outboundListInfo.data.tour_list : '';
    console.log('outboundListInfo',list_tour);
    ...
    return (
        <Container>
            <div className="tourtrongnuoc">
                <h4 className="mt-4">Tour OutBound</h4>
...
const mapStateToProps = state => ({
    outboundListInfo: state.outboundListInfo
});
