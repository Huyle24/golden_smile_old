...
import {addToCartAction, fetchInboundTourList, fetchTourList} from "../../../redux/actions";
import Swal from "sweetalert2";

function InboundTour(props) {
    useEffect(() => {
        props.fetchInboundTourList('', '', '', '', 0, '', '', '', 1)
    }, [])

    let list_tour = props.tourInboundListInfo.data && props.tourInboundListInfo.isLoading === false ? props.tourInboundListInfo.data.tour_list : '';
    ...
    return (
        <div className="Tour_trekking_content">
            <div className="text-center pt-9">
                <div className="TheTourTrekking">Tour Inbound</div>
                                ...
                                <div className="card mx-2 mt-2 trekking_card">
                                    <Link href={"/Tour?id=" + item.id} onClick={() => Product_watched(item)}>
                                        <div className="card-image Trekking-img">
                                            <img  src={item.bucket_img  ? item.bucket_img : 'https://vigomanager.com/app-assets/mobile/img-huy/golden%20smile%20logo.png' }/>
                                            <div className="item-content-price">
                                                <span
                                                    className="item-label price_trekking">{item.price_1_person}đ</span>
                                            </div>
                                        </div>
                                    </Link>
