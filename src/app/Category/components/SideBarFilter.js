...
function SidaBarFilter(props) {
    ...
    return (<>
            <Col xs={12} className={'hide-mobile'}>
                <button className="sidebar-toggle-btn btn" onClick={toggleSidebar}>
                    <i className='bx bx-filter'></i>
                </button>
            </Col>

            <div className={`overlay-sider ${showSidebar ? 'show' : ''}`} onClick={toggleSidebar}></div>
            <Col lg="3" className={`ps-0 col-side-bar ${showSidebar ? 'show' : ''}`}>
                    ...
                    <Form className="px-3 py-4">
                        ...
                        <Form.Select aria-label="Default select example" className="mb-3" onChange={handleTypeTourism}>
                            ...
                            {type_tourism_list && type_tourism_list.map((item, index) => (
                                <option key={index} value={item.id}
                                        selected={typeTourismParam == item.id}>{item.name}</option>
                            ))}
                        ...
                        <Form.Select aria-label="Default select example" className="mb-3" onChange={handleCountryStart}>
                            ...
                            {country_list && country_list.map((item, index) => (
                                <option key={index} value={item.id}
                                        selected={countryParam == item.id}>{item.name}</option>
                            ))}
                        </Form.Select>
