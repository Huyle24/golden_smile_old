...
const ajaxControl = (user_token, action) =>
    ajax({
        ...
        map(response => ({
            type: actionTypes.FETCH_LIST_TYPE_TOURISM_SUCCESS,
            payload: response.response
        })),
        catchError(error => {
            let data_error = {
                type: actionTypes.FETCH_LIST_TYPE_TOURISM_ERROR,
...
const fetchProductListEpic = (action$, store) => action$.pipe(
    ofType(actionTypes.FETCH_LIST_TYPE_TOURISM),
    concatMap(action => of(actionTypes.GET_TOKEN()).pipe(
        map(res => {
            token = JSON.parse(res);
            api_url = actionTypes.BASE_URL_API + "manager/Tour/listTypeTourism";
        })
